...
CreateThread(function()
    ...
    DebugPrint(('Loaded %d garage(s)'):format(#(function()
        local t = {}
        for _ in pairs(loadedGarages) do t[#t+1] = true end
        return t
    end)()))
end)
...
lib.callback.register('sovex_garage:cb:getVehicles', function(src, garageName)
    ...
    -- state = 0 means stored in garage; garage field stores the garage name
...
RegisterNetEvent('sovex_garage:sv:takeVehicle', function(data)
    ...
    -- Resolve spawn point (prefer server-stored spawn over client-sent)
...
-- =====================================================
-- EVENT: SAVE DAMAGE (call from client on vehicle delete / resource stop)
-- =====================================================
...
RegisterNetEvent('sovex_garage:sv:saveDamage', function(data)
    ...
    MySQL.update.await(
        ...
        {
            json.encode(damage),
            data.garageName or 'garage_pillbox',
            plate,
            citizenid,
        }
    )
